...
vec3 applyColorPalette(vec3 color, int palette) {
  ...
  } else if (palette == 5) { // Sunset Blvd (Custom Gradient Map)
    // Colors from reference image
    vec3 c1 = vec3(0.10, 0.08, 0.14); // #1A1423
    vec3 c2 = vec3(0.29, 0.10, 0.26); // #4A1942 (Purple - Target for Background)
    vec3 c3 = vec3(0.70, 0.22, 0.32); // #B33951 (Red)
    vec3 c4 = vec3(0.96, 0.66, 0.38); // #F5A962 (Orange)
    vec3 c5 = vec3(1.00, 0.96, 0.88); // #FFF4E0 (Cream)
    
    // ADJUSTED GRADIENT MAP
    // We widened the "Purple" zone (0.2 to 0.5) so the Gray background (lum ~0.36) 
    // lands firmly in the purple/lavender range.
    if (lum < 0.20) return mix(c1, c2, lum / 0.20);
    if (lum < 0.50) return mix(c2, c3, (lum - 0.20) / 0.30);
    if (lum < 0.80) return mix(c3, c4, (lum - 0.50) / 0.30);
    return mix(c4, c5, (lum - 0.80) / 0.20);
  }
  return color;
}
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  // PALETTE LOGIC
  if (colorPalette == 5) { // Sunset Blvd
     // 1. Get the target color from the palette based on cell brightness
     vec3 paletteColor = applyColorPalette(vec3(brightness), colorPalette);
     
     // 2. BLENDING MODE FIX:
     // Instead of fading to black, we mix between a "dimmed" version of the color 
     // and the "full" version based on the character shape.
     // This means empty space (background) stays purple (just slightly darker),
     // matching the "Lavender Background" look.
     finalColor = mix(paletteColor * 0.6, paletteColor * 1.2, charValue);
     
  } else {
     // Standard behavior for other palettes
     if (colorMode) {
...
export const AsciiEffect = forwardRef((props: any, ref) => {
  ...
  const effect = useMemo(
    ...
    [cellSize, invert, color, styleNum, postfx, resolution, mousePos]
  )
